...
* 本文是 `update.command` 的脚本自述文件。

* 提供 update 函数，以菜单方式更新 Homebrew、Android、Flutter、Node、Rust、Python、Ruby 等工具链。
...
* 脚本文件：`update.command`
* 所属目录：`Scripts/update.command/`
* 推荐入口：`source "$JOBS_MAC_ENV_HOME/Scripts/update.command/update.command"`
...
```shell
source "$JOBS_MAC_ENV_HOME/Scripts/update.command/update.command"
```
...
`update.command` 采用独立目录管理，便于阅读、维护、迁移和长期扩展。
