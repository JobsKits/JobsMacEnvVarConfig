...
jobs_config_show_readme_and_wait() {
...
这是 config.command 的内置自述，不读取同级 README.md。

功能：
  用 Xcode 或系统文本编辑器打开 ~/.zshrc 或指定配置文件。

结构：
  Scripts/config.command/config.command
  Scripts/config.command/README.md
...
EOFREADME
  ...
  if [[ -t 0 && "${JOBS_MAC_ENV_SKIP_README:-}" != "1" ]]; then
    log ""
    warm_echo "按回车继续执行 config..."
    local _answer=""
    IFS= read -r _answer
  fi
}
# 封装 jobs_config_source_lib 对应的独立处理逻辑。
jobs_config_source_lib() {
  local lib_name="$1"
  local candidate=""
  local candidates=(
...
show_script_intro_and_wait() {
  print -r -- '============================== 脚本内置自述 =============================='
  print -r -- '脚本名称：config.command'
  print -r -- '核心用途：执行“config”对应的自动化任务。'
...
initialize_script_module() {
  ...
  setopt NO_NOMATCH
  : > "$LOG_FILE"
  jobs_config_source_lib "jobs_flutter_lib.zsh" || { return 1 2>/dev/null || exit 1; }
