...
jobs_save_show_readme_and_wait() {
...
这是 save.command 的内置自述，不读取同级 README.md。

功能：
  重新加载 bash / zsh 常见配置文件。

结构：
  Scripts/save.command/save.command
  Scripts/save.command/README.md
...
EOFREADME
  ...
  if [[ -t 0 && "${JOBS_MAC_ENV_SKIP_README:-}" != "1" ]]; then
    log ""
    warm_echo "按回车继续执行 save..."
    local _answer=""
    IFS= read -r _answer
  fi
}
# 封装 jobs_save_source_lib 对应的独立处理逻辑。
jobs_save_source_lib() {
  local lib_name="$1"
  local candidate=""
  local candidates=(
...
show_script_intro_and_wait() {
  print -r -- '============================== 脚本内置自述 =============================='
  print -r -- '脚本名称：save.command'
  print -r -- '核心用途：执行“save”对应的自动化任务。'
...
initialize_script_module() {
  ...
  setopt NO_NOMATCH
  : > "$LOG_FILE"
  jobs_save_source_lib "jobs_session_lib.zsh" || { return 1 2>/dev/null || exit 1; }
