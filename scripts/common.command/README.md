...
* 本文是 `common.command` 的脚本自述文件。

* 提供 JobsMacEnv 共享基础函数，例如命令存在性检测和 Homebrew 第三方更新。
...
* 脚本文件：`common.command`
* 所属目录：`Scripts/common.command/`
* 推荐入口：`source "$JOBS_MAC_ENV_HOME/Scripts/common.command/common.command"`
...
graph TD
    ...
    B --> C[检测命令是否存在]
    C --> D[按需执行传入命令]
    D --> E[封装 brew update / upgrade / cleanup / doctor]
    E --> F[供其他模块复用]
    F --> Z([结束])
```
...
### 3、封装 brew update / upgrade / cleanup / doctor

封装 brew update / upgrade / cleanup / doctor。
...
```shell
Scripts/common.command/common.command
```
...
```shell
source "$JOBS_MAC_ENV_HOME/Scripts/common.command/common.command"
```
...
`common.command` 采用独立目录管理，便于阅读、维护、迁移和长期扩展。
