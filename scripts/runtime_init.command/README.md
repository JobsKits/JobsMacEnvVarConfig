...
* 本文是 `runtime_init.command` 的脚本自述文件。

* 提供 JobsMacEnv 启动时需要注册的基础运行时逻辑。
...
* 脚本文件：`runtime_init.command`
* 所属目录：`Scripts/runtime_init.command/`
* 推荐入口：`source "$JOBS_MAC_ENV_HOME/Scripts/runtime_init.command/runtime_init.command"`
...
```shell
Scripts/runtime_init.command/runtime_init.command
```
...
```shell
source "$JOBS_MAC_ENV_HOME/Scripts/runtime_init.command/runtime_init.command"
```
...
`runtime_init.command` 采用独立目录管理，便于阅读、维护、迁移和长期扩展。
