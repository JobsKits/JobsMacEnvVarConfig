# **MacOS** JobsMacEnv - **Shell 会话辅助函数模块**
...
* 本文是 `session.command` 的脚本自述文件。

* 提供重载配置、打开常用配置文件、启动模拟器等会话级快捷命令。
...
* 脚本文件：`session.command`
* 所属目录：`Scripts/session.command/`
* 推荐入口：`source "$JOBS_MAC_ENV_HOME/Scripts/session.command/session.command"`
...
```shell
Scripts/session.command/session.command
```
...
```shell
source "$JOBS_MAC_ENV_HOME/Scripts/session.command/session.command"
```
...
`session.command` 采用独立目录管理，便于阅读、维护、迁移和长期扩展。
