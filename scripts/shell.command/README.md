...
* 本文是 `shell.command` 的脚本自述文件。

* 扫描当前机器可用 shell，并辅助切换默认登录 shell。
...
* 脚本文件：`shell.command`
* 所属目录：`Scripts/shell.command/`
* 推荐入口：`source "$JOBS_MAC_ENV_HOME/Scripts/shell.command/shell.command"`
...
graph TD
    A([开始]) --> B[显示自述文件并等待确认]
    B --> C[扫描 /etc/shells 和常见 shell 路径]
    C --> D[生成候选列表]
    D --> E[选择目标 shell]
    E --> F[执行 chsh 切换]
    F --> Z([结束])
...
### 1、扫描 /etc/shells 和常见 shell 路径

扫描 /etc/shells 和常见 shell 路径。
...
```shell
source "$JOBS_MAC_ENV_HOME/Scripts/shell.command/shell.command"
```
...
`shell.command` 采用独立目录管理，便于阅读、维护、迁移和长期扩展。
