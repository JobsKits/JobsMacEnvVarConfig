...
jobs_check_show_readme_and_wait() {
...
这是 check.command 的内置自述，不读取同级 README.md。

功能：
  进入默认 Flutter 项目，锁定 FVM 版本并执行 flutter doctor -v。

结构：
  Scripts/check.command/check.command
  Scripts/check.command/README.md
...
EOFREADME
  ...
  if [[ -t 0 && "${JOBS_MAC_ENV_SKIP_README:-}" != "1" ]]; then
    log ""
    warm_echo "按回车继续执行 check..."
    local _answer=""
    IFS= read -r _answer
  fi
}
# 封装 jobs_check_source_lib 对应的独立处理逻辑。
jobs_check_source_lib() {
  local lib_name="$1"
  local candidate=""
  local candidates=(
...
show_script_intro_and_wait() {
  print -r -- '============================== 脚本内置自述 =============================='
  print -r -- '脚本名称：check.command'
  print -r -- '核心用途：执行“check”对应的自动化任务。'
...
initialize_script_module() {
  ...
  setopt NO_NOMATCH
  : > "$LOG_FILE"
  jobs_check_source_lib "jobs_flutter_lib.zsh" || { return 1 2>/dev/null || exit 1; }
