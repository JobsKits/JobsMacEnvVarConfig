...
这是 a.command 的内置自述，不读取同级 README.md。

功能：
  打开 ~/.bash_profile。

结构：
  Scripts/a.command/a.command
  Scripts/a.command/README.md
...
# 封装 jobs_a_source_lib 对应的独立处理逻辑。
jobs_a_source_lib() {
  local lib_name="$1"
  local candidate=""
  local candidates=(
...
show_script_intro_and_wait() {
  print -r -- '============================== 脚本内置自述 =============================='
  print -r -- '脚本名称：a.command'
  print -r -- '核心用途：执行“a”对应的自动化任务。'
...
initialize_script_module() {
  ...
  setopt NO_NOMATCH
  : > "$LOG_FILE"
  jobs_a_source_lib "jobs_session_lib.zsh" || { return 1 2>/dev/null || exit 1; }
