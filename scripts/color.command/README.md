...
* 本文是 `color.command` 的脚本自述文件。

* 提供 cor 函数，用于 Hex、RGB、RGBA 等常见颜色格式互转。
...
* 脚本文件：`color.command`
* 所属目录：`Scripts/color.command/`
* 推荐入口：`source "$JOBS_MAC_ENV_HOME/Scripts/color.command/color.command"`
...
```shell
Scripts/color.command/color.command
```
...
```shell
source "$JOBS_MAC_ENV_HOME/Scripts/color.command/color.command"
```
...
`color.command` 采用独立目录管理，便于阅读、维护、迁移和长期扩展。
