...
* 本文是 `path.command` 的脚本自述文件。

* 提供路径处理、拖入路径解析、目录跳转等基础能力。
...
* 脚本文件：`path.command`
* 所属目录：`Scripts/path.command/`
* 推荐入口：`source "$JOBS_MAC_ENV_HOME/Scripts/path.command/path.command"`
...
```shell
Scripts/path.command/path.command
```
...
```shell
source "$JOBS_MAC_ENV_HOME/Scripts/path.command/path.command"
```
...
`path.command` 采用独立目录管理，便于阅读、维护、迁移和长期扩展。
